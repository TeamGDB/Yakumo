...
`Author` is not a documented key; it is read when present. The lists of which file belongs to which piece of equipment are the manager's and the community's data and were not copied: the player types an equipment mod's target file id, or the Mods page works it out from the running game's own tables (`profiles/mhp3rd/host/game/equipment_models.cpp`, traced as [EQUIPMENT_MODS.md](EQUIPMENT_MODS.md) describes; the community file list's armor ids served only as a hint for where to search memory). The obfuscation code in `mhp3rd_data_bin.cpp` is this project's own, from [DATA_BIN.md](DATA_BIN.md). Preview images are decoded with stb_image, as for texture packs. No mod, and no file of the game, is in the repository; the tests build their own from random data.
