...
const std::vector<Field> &fields() {
    static const std::vector<Field> table = {
        ...
        {"input.vertical_camera", "MHP3RD_VERTICAL_CAMERA",
        ...
        {"input.vertical_write", "MHP3RD_VERTICAL_WRITE",
         [](Settings &s, const std::string &t) {
             float value = 0.0f;
             if (!parse_float(t, 0.0f, 1.0f, value)) return false;
             s.vertical_write = static_cast<int>(value);
             return true;
         },
         [](const Settings &s) { return std::to_string(s.vertical_write); },
         [](Settings &s, const char *t) { s.vertical_write = static_cast<int>(variable_float(t, 0.0f, 0.0f, 1.0f)); }},
